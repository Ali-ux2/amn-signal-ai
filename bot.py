...
async def signal_cmd(update, context):
    ...
    for pair in PAIR_MAP.keys():
        try:
            ...
            failed.append(f"{pair}: {type(exc).__name__}")
            continue
        if cand is None:
            skipped.append(pair)
            continue
        if best is None or cand.confidence > best.confidence:
            best = cand
    if best is None:
        lines = ["No card this minute."]
        if failed:
            lines.append("Yahoo fetch failed:")
            lines.extend(failed[:7])
        if skipped:
            lines.append("Filter skipped: " + ", ".join(skipped))
        if not failed and not skipped:
            lines.append("No pairs in PAIR_MAP.")
