...
def format_signal_caption(sig):
    ...
    name = sig.display_pair.replace("USD", "USD/").replace("USD/B", "USD/B")
    if name.startswith("USD/") is False and name.startswith("USD"):
        name = "USD/" + name[3:]
    return (
        f"🚀 <b>Signal ready</b>\n"
        f"<b>{name}</b>\n\n"
...
async def signal_cmd(update, context):
    ...
    try:
        ...
        await context.bot.send_photo(chat_id=CHANNEL, photo=chart, caption=caption, parse_mode=ParseMode.HTML)
    except Exception:
