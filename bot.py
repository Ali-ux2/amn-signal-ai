...
async def post_card(bot, sig, entry_time):
    ...
    await bot.send_message(
        chat_id=CHANNEL,
        text=format_signal_caption(sig),
...
async def start(update, context):
    ...
    await update.message.reply_text(
        f"{BRAND} online.\n"
        f"Auto is on. Feedback comes here.\n"
        f"Cards go to {CHANNEL}.\n"
        f"/signal = extra scan"
    )
...
async def signal_cmd(update, context):
    ...
    try:
        ...
        if best is None:
            await status.edit_text(f"No setup {entry_time}. Auto keeps trying.")
            return
        try:
        ...
        try:
            ...
            if best is None:
                ...
                try:
                    ...
                    await notify(app.bot, f"Channel post failed: {type(exc).__name__}")
        except Exception as exc:
            await notify(app.bot, f"Auto error: {type(exc).__name__}")
            log.warning("auto: %s", exc)
        finally:
            _clear_busy()
